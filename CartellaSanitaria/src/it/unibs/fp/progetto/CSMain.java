...
public class CSMain implements Serializable{
	...
	private static final String E_MEX_INS_ERELATIVO = "Inserisci il nome della tipologia di esame: ";
	...
	public static EsameEffettuato creaEsamePrenotato(ListaEsame listaE, ArrayList<Malattia> listaM){
		...
		switch(scelta){
			case 1:	//Diagnostico
				...
				do{
					do{
						String nomeEsameAss = MyInput.leggiStringaNonVuota(E_MEX_INS_ERELATIVO);
						if(listaE.isEsistente(nomeEsameAss)){
							eAss = (EsameDiagnostico) listaE.cercaEsame(nomeEsameAss);
							valido = true;
						}
			...
			switch(scelta){
				...
				case 2:
					...
					do{
						...
						catch(IllegalArgumentException e){ //se l'esame non Ã¨ coerente con la malattia
							stampaMex(e.getMessage());
						}
					}while(valido = false);
					break;
	...
	*/
	...
	public static void creazioneGuidata(CartellaSanitaria CS, ListaEsame listaE){
		...
		if(sceltaED!=0){//E' stato scelto un esame diagnostico su cui creare i 3 effettuati
	 ...
	 * 					gestire max e min multipli (presumo si intenda piÃ¹ valori uguali in date diverse) (fatto)
	 * 				visualizzare valore medio (media dei valori o valore piÃ¹ vicino alla media? presumo la prima)
	 *  		Modifica Cartella
	...
	public static void main(String[] args) {
		...
		boolean visualizzato = false;//controlla se il to string semplice della cartella Ã¨ stato visualizzato
